...
export default async function handler(req: NextRequest) {
  ...
  return fetch(
    `https://recaptchaenterprise.googleapis.com/v1/projects/gregturnerpianis-1719090524884/assessments?key=${env.RECAPTCHA_KEY}`,
    {
      method: 'POST',
      body: JSON.stringify({
        event: {
    ...
    .then((recaptchaResponse) => {
      ...
      if (
        ...
        return fetch(
          `https://sheets.googleapis.com/v4/spreadsheets/1OqJd30zaq04VIEfeGyrbDIVCRUUttsjBJZOTfRXZtek/values/'Signups'!A1:F1:append?valueInputOption=USER_ENTERED&key=${env.SHEETS_KEY}`,
          {
            method: 'POST',
            body: JSON.stringify({
              range: `'Signups'!A1:F1`,
